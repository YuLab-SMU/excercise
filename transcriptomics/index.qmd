...
上面折叠代码运行时间较长，可以直接加载`airway`包中的示例数据`gse`。
...
`gse`是一个存储为`SummarizedExperiment`结构的数据，其中的基因定量丰度表, 样本信息以及基因信息的访问，具体操作如下。
...
使用`translate_ggplot`需要用户有相应翻译源的`appid`与`key`。可以自行搜索“如何申请百度翻译API，或者参考[fanyi包的README文档](https://github.com/YuLab-SMU/fanyi)，从而获得`appid`与`key`”。再
使用`set_translate_option(appid = "appidxxxxxxx", key="keyxxxxxxxxx", source = 'baidu')`
